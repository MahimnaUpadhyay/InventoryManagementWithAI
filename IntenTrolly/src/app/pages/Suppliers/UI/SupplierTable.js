...
import {getSupplierEndPoint, posSuppliertEndPoint } from '@/app/utility/Supplier_End_Point.js';
...
// COMPONENTS IMPORTS
import SupplierAddModal from './SupplierAddModel';
    ...
    const getSupplier = async () => {
        try {
            ...
            if (!Array.isArray(SupplierData)) {
                ...
                const Array_Data = Object.values(SupplierData);

                if (!Array.isArray(Array_Data)) {
                    console.log("Not an array");
                } else {
                    setSupplier_Data(Array_Data);
                    setLoading(false)
                }
            } else {
                setSupplier_Data(SupplierData);
                setLoading(false)
            }
        } catch (error) {
            console.log("There is an error while getting suppliers", error);
        }
    };


    // SHOW TOTAL COUNT OF PRODUCTS
    const Total_Supplier = async () => {
        try {
            const request = await axios.get(`${BASE_URL}${getSupplierEndPoint}`);
            const SupplierData = request.data.response
                    ...
                    <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                        <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                            ...
                            <div className="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                                ...
                                {isAddModalOpen && <SupplierAddModal closeModal={closeAddModal} />}
                            </div>
                        </div>
                            ...
                            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                <thead className="text-xs text-white uppercase bg-blue-700 dark:bg-gray-700 dark:text-gray-400">
                                    <tr>
                                        ...
                                        <th scope="col" className="px-4 py-3">Supplier Address</th>
                                        <th scope="col" className="px-4 py-3">Supplier Phone No</th>
                                        <th scope="col" className="px-4 py-3">Supplier Rate</th>
                                        <th scope="col" className="px-4 py-3">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {Supplier_Data?.map((data) => (
                                        <>
                                            <tr className="border-b dark:border-gray-700">
                                                <th scope="row" className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{data?.supplier_ID}</th>
                        ...
                        <nav className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
                            <span className="text-sm font-normal text-gray-500 dark:text-gray-400">
                                Showing
                                <span className="font-semibold text-gray-900 dark:text-white ml-1 mr-1">{TotalSupplier}</span>
                                of
                                <span className="font-semibold text-gray-900 dark:text-white ml-1 mr-1">{TotalSupplier}</span>
                            </span>
